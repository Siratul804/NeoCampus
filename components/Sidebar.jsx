...
import { AlignJustify, AlignLeft, Home, UtensilsCrossed, Truck, MapPinned, Calendar, BookOpen, Bot } from "lucide-react"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <span className={`${isExpanded ? "block" : "hidden"} text-lg font-bold`}>
          <Link href="/" className="text-3xl font-bold items-center space-x-2">
            <Image src={logo || "/placeholder.svg"} alt="logo" className="h-auto w-[12vh]" />
          </Link>
        </span>
        <Button
          variant="ghost"
          size="icon"
          className="text-slate-600 hover:text-slate-800 hover:bg-slate-100 mt-5 ml-3"
            ...
            <Link href={item.path} passHref className="w-full">
              <div
                className={`
                  ...
                  ${pathname === item.path ? "bg-slate-100 text-slate-800 font-medium" : "text-slate-600 font-medium"}
                  hover:bg-slate-100
                `}
              >
                <span
                  className={`
                    ...
                    ${pathname === item.path ? "text-blue-600" : "text-slate-500"}
                  `}
                >
                  {item.icon}
                </span>
                <span className={`${isExpanded ? "block" : "hidden"} flex-1 text-sm`}>{item.label}</span>
              </div>
            </Link>
          </li>
        ))}
